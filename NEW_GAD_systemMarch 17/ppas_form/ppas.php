...
.swal-blur-container {
    backdrop-filter: blur(5px);
        ...
        /* Add these styles in the head section, inside the style tag */
        .autocomplete-item {
            padding: 8px;
            font-size: 14px;
        }
        ...
        #existing-programs-container .list-group-item {
        ...
        [data-bs-theme="dark"] #existing-programs-container {
        ...
        /* Improve the focus style of the gender issue input */
        #gender_issue:focus {
            border-color: var(--accent-color);
...
<body>
                ...
                <form id="ppasForm">
                    ...
                    <div class="form-page" id="page1">
                        <div class="row g-3">
                            ...
                            <div class="col-md-12">
                                <label for="gender_issue" class="form-label">Gender Issue/s</label>
                    ...
                    <div class="form-page" id="page4" style="display: none;">
                        <div class="row g-3">
                            ...
                            <div class="col-md-12">
                                <label for="psAttribution" class="form-label">PS Attribution (BatStateU Participants)</label>
                                <div class="input-group">
                                    <span class="input-group-text">₱</span>
                                    <input type="number" class="form-control" id="psAttribution" name="psAttribution" step="0.01" min="0" readonly>
                                    <button type="button" class="btn btn-outline-secondary" data-bs-toggle="tooltip" title="PS Attribution = Sum of (Hourly Rate × 8 × Number of Personnel) per Academic Rank">
                                        <i class="fas fa-info-circle"></i>
                                    </button>
                                </div>
                                <div class="small text-muted mt-1">Calculated based on hourly rates of personnel involved</div>
                            </div>
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', async function() {
            ...
            async function checkExistingForm() {
                // Removed validation for year/quarter uniqueness
                // Multiple programs/projects can now be saved under the same year and quarter
            }
            
            yearSelect.addEventListener('change', function() {
                // No validation needed, just keep the event listener
            });
        ...
        function addPersonnelToList(role, person) {
            ...
            // Calculate PS Attribution based on updated personnel list
        ...
        document.getElementById('ppasForm').addEventListener('submit', async function(e) {
            ...
            // Check for required fields (except gender_issue)
            if (!year || !quarter || !title || !location || !startDate || !endDate || !startTime || !endTime) {
                Swal.fire({
            ...
            const formData = {
                year: year,
                quarter: quarter,
                gender_issue: $('#gender_issue').val(), // Can be empty or duplicate now
                ...
                startTime: startTime,
                endTime: endTime,
                hasLunchBreak: $('#hasLunchBreak').is(':checked'),
                totalDuration: $('#totalDuration').val(),
                rawTotalDuration: $('#rawTotalDuration').val(),
                approvedBudget: $('#approvedBudget').val(),
                sourceOfBudget: $('#sourceOfBudget').val(),
                psAttribution: $('#psAttribution').val(),
                
                // Get personnel data
                personnel: window.addedPersonnel || {},
                
                // Get beneficiaries data
                beneficiaries: {
                    internalMaleStudents: $('#internalMaleStudents').val(),
                    internalFemaleStudents: $('#internalFemaleStudents').val(),
                    internalMaleFaculty: $('#internalMaleFaculty').val(),
                    internalFemaleFaculty: $('#internalFemaleFaculty').val(),
                    internalOtherGender: $('#internalOtherGender').val(),
                    externalType: $('#externalType').val(),
                    externalMale: $('#externalMale').val(),
                    externalFemale: $('#externalFemale').val()
                },
            ...
            $.ajax({
                ...
                success: function(response) {
                    if (response.success) {
                        Swal.fire({
                            title: 'Success!',
                            text: 'PPAS form saved successfully',
                            icon: 'success',
                            confirmButtonColor: '#6a1b9a'
                        }).then(() => {
                            // Redirect back to the PPAS form page instead of dashboard
                            window.location.href = 'ppas.php';
                        });
                    } else {
                        Swal.fire({
                            title: 'Error!',
                ...
                error: function(xhr, status, error) {
                    ...
                    let errorMessage = 'Failed to save the form. Please try again.';
                    try {
                        ...
                        // If we can't parse the response as JSON, use the raw text (truncated if too long)
                        if (xhr.responseText) {
                            const truncatedResponse = xhr.responseText.substring(0, 100) + 
                                (xhr.responseText.length > 100 ? '...' : '');
                            errorMessage = 'Server error: ' + truncatedResponse;
                        }
                    }
        ...
        function navigateForm(direction) {
            ...
            let validated = validateCurrentPage();
            ...
            setTimeout(() => {
                ...
                // Calculate PS Attribution when showing the budget page
                if (newPage === 4) {
                    calculatePSAttribution();
                    
                    // Initialize tooltip for PS Attribution
                    setTimeout(() => {
        ...
        // This functionality has been removed to allow direct input
        // No title suggestions or autocomplete are needed
        ...
        // Function to calculate PS Attribution
        function calculatePSAttribution() {
            console.log("Calculating PS Attribution");
            
            // Initialize variables to store academic ranks and their counts + hourly rates
            let academicRankData = {};
            
            // Process all personnel categories
            Object.keys(window.addedPersonnel).forEach(role => {
                if (Array.isArray(window.addedPersonnel[role])) {
                    window.addedPersonnel[role].forEach(person => {
                        if (person.academic_rank && person.hourly_rate) {
                            const academicRank = person.academic_rank;
                            const hourlyRate = parseFloat(person.hourly_rate) || 0;
                            
                            // Initialize if this rank hasn't been seen yet
                            if (!academicRankData[academicRank]) {
                                academicRankData[academicRank] = {
                                    count: 0,
                            ...
                            academicRankData[academicRank].count++;
                        }
                    });
                }
            });
            
            // Calculate PS Attribution: Sum of (Hourly Rate × 8 × Number of Personnel) per Academic Rank
            let totalPSAttribution = 0;
            
            Object.keys(academicRankData).forEach(rank => {
                const { count, hourlyRate } = academicRankData[rank];
                const rankAttribution = hourlyRate * 8 * count;
                totalPSAttribution += rankAttribution;
                
                console.log(`${rank}: ${count} personnel × ${hourlyRate} hourly rate × 8 = ${rankAttribution}`);
            });
            
            // Update the PS Attribution field
            $('#psAttribution').val(totalPSAttribution.toFixed(2));
            
            console.log(`Total PS Attribution: ${totalPSAttribution.toFixed(2)}`);
            
            return totalPSAttribution;
        }
    </script>
    ...
    <script>
        // Initialize the gender issue autocomplete
        $(document).ready(function() {
            $("#gender_issue").autocomplete({
                source: function(request, response) {
                    $.ajax({
                        url: "search_gender_issues.php",
                        dataType: "json",
                        data: {
                            term: request.term
                        },
                ...
                select: function(event, ui) {
                    ...
                    // After selecting a gender issue, call the function to show existing programs/projects
                    showExistingPrograms(ui.item.value);
                    return false;
                }
            }).autocomplete("instance")._renderItem = function(ul, item) {
                return $("<li>")
            ...
            // Also trigger the search when the field changes manually
            $("#gender_issue").change(function() {
                const selectedIssue = $(this).val();
                if (selectedIssue) {
                    showExistingPrograms(selectedIssue);
                } else {
                    // Clear existing programs display
                    $("#existing-programs-container").remove();
                }
            });

            // Type radio buttons - update title label based on selection
            $('input[name="type"]').change(function() {
                const selectedType = $('input[name="type"]:checked').val();
                if (selectedType === "Program") {
                    $('label[for="title"]').text("Title of Implemented Program");
                } else {
                    $('label[for="title"]').text("Title of Implemented Project");
                }
            });
            
            // Set the initial label based on the default selected type
            const initialType = $('input[name="type"]:checked').val();
            if (initialType === "Program") {
                $('label[for="title"]').text("Title of Implemented Program");
            } else {
                $('label[for="title"]').text("Title of Implemented Project");
            }
        ...
        // Function to show existing programs/projects under the selected gender issue
        function showExistingPrograms(genderIssue) {
            // If gender issue is empty, don't proceed
            if (!genderIssue) {
                return;
            }
            ...
            let placeholder = '<div id="existing-programs-container" class="mt-3 mb-3 p-3 border rounded bg-light">';
            placeholder += '<p class="text-center"><i class="fas fa-spinner fa-spin"></i> Loading existing programs...</p>';
            placeholder += '</div>';
            
            // Insert the placeholder after the gender issue field
            $('#gender_issue').after(placeholder);

            $.ajax({
                url: 'get_existing_programs.php',
                method: 'POST',
                data: { gender_issue: genderIssue },
                dataType: 'json',
                success: function(data) {
                    // Remove the placeholder
                    $("#existing-programs-container").remove();
                    
                    // Create content based on what we got
                    let html = '<div id="existing-programs-container" class="mt-3 mb-3 p-3 border rounded bg-light">';
                    
                    if (data.length > 0) {
                        // Show existing programs/projects
                        html += '<h6 class="mb-2">Existing programs/projects under this gender issue:</h6>';
                        html += '<ul class="list-group">';

                        data.forEach(item => {
                            // Create a nice list item for each program/project
                            html += `<li class="list-group-item d-flex justify-content-between align-items-center">
                                ${item.title}
                                <span class="badge ${item.type === 'Program' ? 'bg-primary' : 'bg-success'} rounded-pill">${item.type}</span>
                            </li>`;
                        });

                        html += '</ul>';
                        html += '<div class="mt-2 small text-muted">You can add another program/project under this gender issue.</div>';
                    } else {
                        // No existing programs found
                        html += '<p>No existing programs/projects found for this gender issue.</p>';
                        html += '<div class="mt-2 small text-muted">You are creating the first program/project for this gender issue.</div>';
                    }
                    
                    html += '</div>';
                    
                    // Insert the HTML after the gender issue field
                    $('#gender_issue').after(html);
                },
                error: function(xhr, status, error) {
                    // Remove the placeholder
                    $("#existing-programs-container").remove();
                    ...
                    let html = '<div id="existing-programs-container" class="mt-3 mb-3 p-3 border rounded bg-light text-danger">';
                    html += '<p><i class="fas fa-exclamation-triangle"></i> Error loading existing programs.</p>';
                    html += '<small>This does not affect your ability to submit the form.</small>';
                    html += '</div>';
        ...
        // REMOVE DUPLICATE FORM HANDLER: There's already a form submission handler defined earlier
        // using document.getElementById('ppasForm').addEventListener('submit', async function(e) {...}
        // This duplicate handler was causing the form to be reset before it finished saving
    </script>
</body>
</html>