...
.modal-dialog {
    max-height: 90vh; /* Increase modal height to 90% of viewport height */
    margin: 5vh auto; /* Center vertically with 5vh margin top and bottom */
    z-index: 1056 !important;
}
...
.table-container {
    height: calc(90vh - 280px); /* Adjust for modal header, filters, and padding */
    display: flex;
    flex-direction: column;
}
                ...
                <div class="col-md-6 col-xl-3">
                    <div class="analytics-card">
                        ...
                        <div class="analytics-dots">
                            ...
                            <span class="dot" onclick="showGenderSlide(2)"></span>
                        </div>
                    </div>
                </div>
            </div>
...
function handleLogout(event) {
    ...
    if (currentUser === 'Central') {
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            const otherGenderRadio = document.getElementById('other');
            ...
            // Other gender handler
            otherGenderRadio.addEventListener('change', function() {
                // Always keep the input visible, just update interactivity
                // otherGenderInput.style.display = this.checked ? 'block' : 'none';
...
function clearAcademicRankDetails() {
    ...
    document.getElementById('hourlyRate').value = '';
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            // Initially disable the other gender input
            otherGenderInput.disabled = true;
            otherGenderInput.required = false;

            // Add change event listeners to all gender radio buttons
            genderRadios.forEach(radio => {
                radio.addEventListener('change', updateOtherGenderVisibility);
            });
            
            // Check initial state in case a radio button is already checked
            updateOtherGenderVisibility();
        });
...
function fillFormWithPersonData(person) {
    ...
    if (genderValue === 'male' || genderValue === 'female') {
        ...
        // Don't clear the other gender input, it will be disabled but preserve the value
    } else {
        // For any non-standard gender value, select "Other" and set the input field
        document.getElementById('other').checked = true;
        otherGenderInput.value = person.gender;
    }
    ...
    console.log('Other gender input value:', otherGenderInput.value);
    console.log('Other gender input disabled:', otherGenderInput.disabled);
...
function exitEditMode() {
    ...
    if (window.originalFormData) {
        ...
        if (window.originalFormData.gender) {
            document.getElementById(window.originalFormData.gender).checked = true;
            if (window.originalFormData.gender === 'other') {
                otherGenderInput.value = window.originalFormData.otherGender;
            }
        }
...
function handleUpdate() {
    ...
    if (selectedGender && selectedGender.value === 'other' && !otherGenderInput.value.trim()) {
        missingFields.push('Other Gender specification');
    }
    ...
    .then(data => {
        ...
        if (data.status === 'success') {
            Swal.fire({
                ...
                // Add a small delay before reloading to ensure server has processed the update
                setTimeout(() => {
...
function updateOtherGenderVisibility() {
    ...
    // Always keep the input visible, just control interactivity
    ...
    if (selectedGender && selectedGender.value === 'other') {
        ...
        otherGenderInput.style.backgroundColor = '';
        otherGenderInput.style.cursor = 'text';
    } else {
        otherGenderInput.disabled = true;
        otherGenderInput.required = false;
        otherGenderInput.style.backgroundColor = '#f5f5f5';
        otherGenderInput.style.cursor = 'not-allowed';
        // Don't clear the value if it's disabled, to preserve the data
    }
    
    console.log('Updated other gender input state. Selected gender:', 
        selectedGender ? selectedGender.value : 'none', 
        'Disabled:', otherGenderInput.disabled);
}
