...
<head>
    ...
    <style>
        ...
        .analytics-card {
            ...
            height: 290px; /* Increased from 260px */
            display: flex;
            flex-direction: column;
            position: relative;
            overflow: hidden;
        ...
        .filter-group select {
            ...
            border-radius: 5px;
            border: 1px solid var(--border-color);
            background: #2B3035; /* Light mode interactive color */
            color: var(--text-primary);
        ...
        .filter-group select:disabled {
            ...
            cursor: not-allowed;
            border: 1px dotted var(--border-color); /* Add dotted border for disabled state */
        }

        [data-bs-theme="dark"] .filter-group select:disabled {
            background-color: #37383A; /* Dark mode disabled color as requested */
            border-color: rgba(255, 255, 255, 0.15);
            border-style: dotted; /* Dotted border in dark mode */
        }
...
@media (max-width: 991px) {
    ...
    .dropdown-submenu > a:after {
        ...
        #budgetChart, #activitiesChart, #beneficiariesChart {
            max-width: 60%;
            max-height: 160px;
            margin: 0; /* Left align charts */
        }
        ...
        .detailed-analysis-section {
            margin-bottom: 1.5rem;
            display: none; /* Hidden by default, will be shown based on chart type */
        }
        ...
        .table-responsive {
            overflow-x: auto;
            max-height: 550px; /* Increased from 450px to 550px for more rows */
            overflow-y: auto;
        }
                        ...
                        <div class="chart-container">
                            ...
                            <div class="chart-values">
                                <div class="chart-value-item">
                                    ...
                                    <div class="chart-value-number proposed-value"><?php echo number_format($proposedBeneficiaries); ?></div>
                                </div>
                                <div class="chart-value-item">
                                    <div class="chart-value-label">Actual Beneficiaries:</div>
                                    <div class="chart-value-number actual-value"><?php echo number_format($actualBeneficiaries); ?></div>
                                </div>
                            </div>
                        </div>
                    </div>
...
.trend-label, .date-label, .quarter-label {
    white-space: nowrap;
    overflow: hidden;
    text-overflow: ellipsis;
}
...
.quarter-label {
    ...
    color: var(--accent-color);
        ...
        @media (max-width: 768px) {
            ...
            .analytics-card {
                height: 290px; /* Increased height for mobile to fit values */
                        ...
                        if (chartContainer) {
                            ...
                            <div class="chart-values">
                                <div class="chart-value-item">
                                    ...
                                    <div class="chart-value-number proposed-value">${chartType === 'budgetChart' ? '₱' + new Intl.NumberFormat().format(data.proposed.toFixed(2)) : new Intl.NumberFormat().format(data.proposed)}</div>
                                </div>
                                <div class="chart-value-item">
                                    ...
                                    <div class="chart-value-number actual-value">${chartType === 'budgetChart' ? '₱' + new Intl.NumberFormat().format(data.actual.toFixed(2)) : new Intl.NumberFormat().format(data.actual)}</div>
                                </div>
                            </div>`;
