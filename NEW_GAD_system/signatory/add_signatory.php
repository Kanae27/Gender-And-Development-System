...
try {
    ...
    $requiredFields = ['name1', 'name2', 'name3', 'name4', 'name5'];
    $missingFields = [];
    ...
    try {
        if ($existingId) {
            ...
            $stmt = $pdo->prepare("INSERT INTO signatories 
                                  (name1, gad_head_secretariat, name2, vice_chancellor_rde, name3, chancellor, name4, asst_director_gad, name5, head_extension_services, campus) 
                                  VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
