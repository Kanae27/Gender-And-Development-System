    ...
    <script>
        ...
        function calculateDuration() {
            ...
            if (startTime && endTime) {
                ...
                let netDuration = Math.max(0, totalDuration - breakDuration);
                
                // Format the output string
                let durationString = totalDuration.toFixed(2);
                if (breakDuration > 0) {
                    durationString = `${totalDuration.toFixed(2)} (Net: ${netDuration.toFixed(2)})`;
                }
        ...
        function addPersonnelToList(role, person) {
            ...
            Object.keys(addedPersonnel).forEach(r => {
                if (Array.isArray(addedPersonnel[r])) {
                    if (addedPersonnel[r].some(p => p.id === person.id)) {
                        isAlreadyAdded = true;
                        existingRole = r;
                    }
                }
